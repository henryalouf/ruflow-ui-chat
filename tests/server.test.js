...
describe('Server — WebSocket', () => {
  ...
  describe('Session management', () => {
    ...
    it('should handle delete_session', async () => {
      ...
      // Delete is deliberately soft: the file stays and is flagged archived, which is
      // what makes the restore path in the UI possible. Asserting the file is gone
      // encoded an older, harder behaviour and had been failing unnoticed.
      const onDisk = path.join(SESSIONS_DIR, `${sessionId}.json`);
      assert.ok(fs.existsSync(onDisk), 'archived session file is retained for restore');
      const archived = JSON.parse(fs.readFileSync(onDisk, 'utf-8'));
