...
function putSearch(key, results) {
  ...
  _searchCache.set(key, { at: Date.now(), results });
}
...
function searchMemory(agentdbSearchScript, query, { limit = 10 } = {}) {
  ...
  // Cache failures too, briefly: a broken search subsystem must not be
  // re-spawned on every context refresh.
  putSearch(cacheKey, results);
  return results;
}
