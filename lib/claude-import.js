...
function toRuflow({ projects = [], conversations = [] } = {}) {
  ...
  for (const p of projects) {
    ...
    const knowledge = docs
      ...
      .map(d => ({
        // claude.ai's API returns file_name (snake_case); the data export uses
        // filename. Reading only one of them silently fell back to the uuid, so
        // every imported knowledge file was listed as a meaningless hex string.
        sourceId: d.uuid || null,
        name: d.file_name || d.filename || d.title || d.uuid || 'untitled',
        mime: 'text/plain',
        text: d.content,
      }));
    ...
    if (existing) {
      ...
      if (!dryRun) {
        try {
          ...
          const have = full && full.knowledge || [];
          const bySource = new Set(have.filter(k => k.sourceId).map(k => k.sourceId));
          const byName = new Set(have.map(k => k.name));
          for (const k of p.knowledge) {
            // sourceId is authoritative; name is only a fallback for entries
            // imported before sourceId was recorded.
            const seen = k.sourceId ? bySource.has(k.sourceId) : byName.has(k.name);
            if (seen) {
              // Same doc, possibly a new filename: update the label rather than
              // leaving the UI showing what it was called at first import.
              if (k.sourceId && typeof store.renameKnowledge === 'function') {
                const existingEntry = have.find(e => e.sourceId === k.sourceId);
                if (existingEntry && existingEntry.name !== k.name) {
                  store.renameKnowledge(existing.id, existingEntry.id, k.name);
                }
              }
              continue;
            }
            store.addKnowledge(existing.id, {
              name: k.name, mime: k.mime, sourceId: k.sourceId,
              buffer: Buffer.from(k.text, 'utf-8'),
            });
          }
          /*
           * updateProject() and addKnowledge() both stamp updatedAt = now(), so
           * a routine re-sync made every project look edited seconds ago and
           * destroyed the recency ordering in the sidebar. The create branch
           * already restores the real dates; the update branch did not.
           */
          if (typeof store.setImportedTimestamps === 'function') {
            store.setImportedTimestamps(existing.id, { createdAt: p.createdAt, updatedAt: p.updatedAt });
          }
        } catch (err) {
      ...
      if (!dryRun) {
        try {
          ...
          for (const k of p.knowledge) {
            try {
              store.addKnowledge(created.id, { name: k.name, mime: k.mime, sourceId: k.sourceId, buffer: Buffer.from(k.text, 'utf-8') });
            } catch (err) {
