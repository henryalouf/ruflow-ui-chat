...
function createStreamEventProcessor(opts) {
  ...
  function persistTurn() {
    ...
    /*
     * Append against the session's current on-disk state, not the snapshot this
     * run has been holding since it started. A concurrent turn on the same
     * session would otherwise have its messages silently dropped by whichever
     * save landed last.
     */
    if (typeof appendSessionMessages === 'function') {
      const updated = appendSessionMessages(session.id, turnMessages, {
        cliSessionId: session.cliSessionId,
        model: session.model,
        name: session.name,
      });
      if (updated) session.messages = updated.messages;
      else { session.messages.push(...turnMessages); saveSession(session); }
    } else {
